...
# ML helper
try:
    from common.ml import load_model, simple_esg
except Exception:
    load_model = None  # type: ignore
    simple_esg = None  # type: ignore
...
if USE_VERTEX and vertexai and GenerativeModel:
    try:
        ...
        logging.getLogger(__name__).warning("Vertex AI init failed, falling back to offline mode: %s", _e)
...
class ESGAnalyzer:
    ...
    def __init__(self):
        ...
        if USE_ML and load_model:
            path = os.getenv("ESG_MODEL_PATH", "models/esg.joblib")
            self.ml_model = load_model(path, kind="esg")
    ...
    async def analyze(self, request: ESGRequest) -> ESGResponse:
        ...
        # ML short-circuit when available
        if USE_ML and self.ml_model and simple_esg:
            parts = simple_esg(request.metadata)
            environmental = EnvironmentalScore(
                score=parts["e"],
                carbon_footprint="Heuristic/ML",
                renewable_energy_usage=50.0,
                waste_management="Heuristic/ML",
                water_usage="Heuristic/ML",
                biodiversity_impact="Heuristic/ML",
            )
            social = SocialScore(
                score=parts["s"],
                employee_welfare="Heuristic/ML",
                diversity_inclusion="Heuristic/ML",
                community_impact="Heuristic/ML",
                human_rights="Heuristic/ML",
                customer_satisfaction="Heuristic/ML",
            )
            governance = GovernanceScore(
                score=parts["g"],
                board_structure="Heuristic/ML",
                executive_compensation="Heuristic/ML",
                shareholder_rights="Heuristic/ML",
                transparency="Heuristic/ML",
                ethics_compliance="Heuristic/ML",
            )
        else:
            # Analyze each ESG component via LLM or fallback
