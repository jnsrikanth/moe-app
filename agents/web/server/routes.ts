...
// Friendly model label mapper for API responses
function friendlyModelLabel(raw?: string): string {
  if (!raw) return '';
  const s = String(raw).toLowerCase();
  if (s.includes('gemini')) {
    const is25 = /2\.5/.test(s);
    const is15 = /1\.5/.test(s);
    const isPro = /\bpro\b/.test(s);
    const isFlash = /flash/.test(s);
    const isLite = /lite/.test(s);
    if (is25 && isFlash && isLite) return 'Google Gemini 2.5 Flash Lite';
    if (is25 && isFlash) return 'Google Gemini 2.5 Flash';
    if (is25 && isPro) return 'Google Gemini 2.5 Pro';
    if (is15 && isFlash) return 'Google Gemini 1.5 Flash';
    if (is15 && isPro) return 'Google Gemini 1.5 Pro';
    if (isPro) return 'Google Gemini Pro';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/model/status', async (req, res) => {
    try {
      // Probe Vertex AI explicitly regardless of current provider
      const { VertexService } = await import('./vertex-service');
      let vertexAvailable = false;
      try {
        const vs = new VertexService();
        vertexAvailable = await vs.testConnection();
      } catch {
        vertexAvailable = false;
      }
      const availableModels = ['local-lightweight'];
      if (vertexAvailable) {
        availableModels.push('vertex-gemini-pro', 'vertex-gemini-lite');
      }
      // Normalize provider: if Vertex is unavailable but provider is set to vertex, switch to local
      let current = aiService.getProvider();
      if (!vertexAvailable && current === 'vertex') {
        aiService.setProvider('local');
        current = 'local';
      }
      res.json({ availableModels, currentProvider: current });
    } catch (error) {
      res.json({ availableModels: ['local-lightweight'], currentProvider: 'local' });
    }
  });
  ...
  app.post('/api/model/switch', async (req, res) => {
    try {
      ...
      const provider = modelId && String(modelId).startsWith('vertex-') ? 'vertex' : 'local';
      if (provider === 'vertex') {
        // Probe Vertex AI explicitly
        const { VertexService } = await import('./vertex-service');
        const ok = await new VertexService().testConnection();
        if (!ok) {
          return res.status(400).json({ error: 'Vertex AI unavailable', fallbackProvider: 'local-lightweight' });
        }
      }
      aiService.setProvider(provider as any);
  ...
  app.get("/api/expert-agents", async (req, res) => {
    try {
      ...
      // Derive model labels dynamically based on current AI provider
      const isLocal = aiService.getProvider && aiService.getProvider() === 'local';
      const normalized = agents.map(a => ({
        ...a,
        model: isLocal ? 'Local Lightweight' : friendlyModelLabel(AGENT_MODELS[(a.type as 'credit' | 'fraud' | 'esg')] || a.model),
      }));
      res.json(normalized);
    } catch (error) {
  ...
  app.get("/api/models", async (req, res) => {
    try {
      const cfg = await storage.getRouterConfig();
      const isLocal = aiService.getProvider && aiService.getProvider() === 'local';
      let routerModel = isLocal ? 'Local Lightweight' : friendlyModelLabel(ROUTER_MODEL);
      ...
      res.json({
        routerModel,
        agents: isLocal ? { credit: 'Local Lightweight', fraud: 'Local Lightweight', esg: 'Local Lightweight' } : {
          credit: friendlyModelLabel(AGENT_MODELS.credit),
          fraud: friendlyModelLabel(AGENT_MODELS.fraud),
          esg: friendlyModelLabel(AGENT_MODELS.esg),
        },
      });
    } catch (error) {
  ...
  async function sendInitialData(ws: WebSocket) {
    if (ws.readyState === WebSocket.OPEN) {
      try {
        ...
        // Normalize agent model labels based on current provider
        const isLocal = aiService.getProvider && aiService.getProvider() === 'local';
        ...
        ws.send(JSON.stringify({
          ...
          data: {
            ...
            registry,
            models: {
              routerModel: (aiService.getProvider && aiService.getProvider() === 'local') ? 'Local Lightweight' : friendlyModelLabel(ROUTER_MODEL),
              agents: (aiService.getProvider && aiService.getProvider() === 'local') ? { credit: 'Local Lightweight', fraud: 'Local Lightweight', esg: 'Local Lightweight' } : {
                credit: friendlyModelLabel(AGENT_MODELS.credit),
                fraud: friendlyModelLabel(AGENT_MODELS.fraud),
                esg: friendlyModelLabel(AGENT_MODELS.esg),
              },
