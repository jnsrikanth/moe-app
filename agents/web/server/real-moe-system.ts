...
Respond in JSON format.`;
  ...
  private async updateAgentMetrics(agentId: string): Promise<void> {
    ...
    let friendlyModel = modelLabelFromRaw(rawModel, instance.agentType);
    // Override label when local provider is active
    try {
      if (aiService.getProvider && aiService.getProvider() === 'local') {
        friendlyModel = 'Local Lightweight';
      }
    } catch {}
...
export const ROUTER_MODEL = process.env.ROUTER_MODEL || process.env.VERTEX_AI_MODEL || process.env.AGENT_MODEL_DEFAULT || 'gemini-1.5-flash';
export const AGENT_MODELS: Record<'credit' | 'fraud' | 'esg', string> = {
  credit: process.env.AGENT_MODEL_CREDIT || process.env.AGENT_MODEL_DEFAULT || process.env.VERTEX_AI_MODEL || ROUTER_MODEL,
  fraud: process.env.AGENT_MODEL_FRAUD || process.env.AGENT_MODEL_DEFAULT || process.env.VERTEX_AI_MODEL || ROUTER_MODEL,
  esg: process.env.AGENT_MODEL_ESG || process.env.AGENT_MODEL_DEFAULT || process.env.VERTEX_AI_MODEL || ROUTER_MODEL,
};
