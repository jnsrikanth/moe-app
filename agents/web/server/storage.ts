...
export class MemStorage implements IStorage {
  ...
  constructor() {
    ...
    // Helper mappers for friendly labels and parameters
    const friendlyModelLabel = (raw?: string): string => {
      if (!raw) return '';
    ...
    const deriveParameters = (friendly: string): string => {
      const s = friendly.toLowerCase();
      if (s.includes('gemini')) {
        if (s.includes('2.5') && s.includes('flash') && s.includes('lite')) return '2.5 Flash Lite';
        if (s.includes('2.5') && s.includes('flash')) return '2.5 Flash';
        if (s.includes('1.5') && s.includes('flash')) return '1.5 Flash';
        if (s.includes('pro')) return 'Pro';
        return '';
      }
      if (/8x7b/i.test(friendly)) return '8x7B';
      if (/70b/i.test(friendly)) return '70B';
      if (/\b8b\b/i.test(friendly)) return '8B';
      return '';
    };

    // Determine base model ids from environment (fallback to 1.5 Flash)
    const baseModelId = process.env.AGENT_MODEL_DEFAULT || process.env.ROUTER_MODEL || process.env.VERTEX_AI_MODEL || 'gemini-1.5-flash';
    const agentModelIds = {
      credit: process.env.AGENT_MODEL_CREDIT || baseModelId,
      fraud: process.env.AGENT_MODEL_FRAUD || baseModelId,
      esg: process.env.AGENT_MODEL_ESG || baseModelId,
    } as const;

    // Initialize with default expert agents using friendly labels
    const defaultAgents: ExpertAgent[] = [
      {
        ...
        type: 'credit',
        status: 'idle',
        model: friendlyModelLabel(agentModelIds.credit),
        parameters: deriveParameters(friendlyModelLabel(agentModelIds.credit)),
        cpuUsage: 15,
        memoryUsage: '2.8GB',
        tokensPerMinute: 890,
        queueLength: 0,
      ...
      },
      {
        ...
        type: 'fraud',
        status: 'idle',
        model: friendlyModelLabel(agentModelIds.fraud),
        parameters: deriveParameters(friendlyModelLabel(agentModelIds.fraud)),
        cpuUsage: 12,
        memoryUsage: '2.1GB',
        tokensPerMinute: 670,
        queueLength: 0,
      ...
      },
      {
        ...
        type: 'esg',
        status: 'idle',
        model: friendlyModelLabel(agentModelIds.esg),
        parameters: deriveParameters(friendlyModelLabel(agentModelIds.esg)),
        cpuUsage: 10,
        memoryUsage: '2.3GB',
        tokensPerMinute: 540,
        queueLength: 0,
