...
for (const r of regions) {
    try {
      ...
      for (const s of svcs) {
        const name = s?.name?.split('/').pop() || 'service';
        let requests5m = 0;
        try {
          const { getRunRequestCount5m } = await import('./monitoring');
          requests5m = await getRunRequestCount5m(projectId, name);
        } catch {}
        const min = s?.template?.scaling?.minInstanceCount ?? 0;
        const runtimeStatus = min > 0 ? (requests5m > 0 ? 'ACTIVE' : 'STANDBY') : 'OFFLINE';
        items.push({
