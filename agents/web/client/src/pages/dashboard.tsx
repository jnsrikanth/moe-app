...
export default function Dashboard() {
  ...
  const [currentProvider, setCurrentProvider] = useState<'local' | 'vertex'>('local');
  const [selectedModelId, setSelectedModelId] = useState<string>('local-lightweight');
  ...
  // Fetch AI model status on load
  useEffect(() => {
    (async () => {
      try {
        const resp = await fetch('/api/model/status');
        if (resp.ok) {
          const { availableModels, currentProvider } = await resp.json();
          const avail = Array.isArray(availableModels) ? availableModels : [];
          setAvailableModels(avail);
          const provider = currentProvider === 'vertex' ? 'vertex' : 'local';
          setCurrentProvider(provider);
          // Choose a consistent selection that matches the provider
          if (provider === 'vertex') {
            const preferred = avail.includes('vertex-gemini-pro')
              ? 'vertex-gemini-pro'
              : (avail.find(m => m.startsWith('vertex-')) || 'local-lightweight');
            setSelectedModelId(preferred);
          } else {
            setSelectedModelId('local-lightweight');
          }
        } else {
          setAvailableModels(['local-lightweight']);
  ...
  async function handleSwitchModel(modelId: string) {
    try {
      ...
      if (!resp.ok) {
        ...
        throw new Error((data as any)?.error || `Switch failed (${resp.status})`);
      }
      setSelectedModelId(modelId);
      // Refresh status
      const stResp = await fetch('/api/model/status');
      if (stResp.ok) {
        const st = await stResp.json();
        setAvailableModels(st.availableModels || []);
        setCurrentProvider(st.currentProvider === 'vertex' ? 'vertex' : 'local');
      }
      toast({ title: 'AI model updated', description: `Using ${modelId}` });
    } catch (e: any) {
      toast({ title: 'Model switch failed', description: e?.message || 'Unknown error', variant: 'destructive' });
      setSelectedModelId('local-lightweight');
      setCurrentProvider('local');
    }
  }
            ...
            <div className="flex items-center space-x-4">
              <div className="bg-gray-800 px-4 py-2 rounded-lg">
                ...
                <div className="flex items-center space-x-2">
                  <Select value={selectedModelId} onValueChange={handleSwitchModel}>
                    <SelectTrigger className="w-[220px] bg-gray-900 border-gray-700 text-white">
                      <SelectValue placeholder="Select model" />
                    </SelectTrigger>
                    <SelectContent className="bg-gray-800 text-white border-gray-700">
                      {availableModels.includes('vertex-gemini-pro') && (
                        <SelectItem value="vertex-gemini-pro">Vertex AI (Gemini Pro)</SelectItem>
                      )}
                      {availableModels.includes('vertex-gemini-lite') && (
                        <SelectItem value="vertex-gemini-lite">Vertex AI (Gemini Lite)</SelectItem>
                      )}
                      <SelectItem value="local-lightweight">Local Lightweight</SelectItem>
                    </SelectContent>
                  </Select>
                  <span className={`text-xs ${currentProvider === 'vertex' ? 'text-green-400' : 'text-yellow-400'}`}>
                    {currentProvider === 'vertex' ? 'Vertex AI' : 'Local'}
                  </span>
                  {currentProvider === 'local' && (
                    <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-yellow-500/10 text-yellow-300 border border-yellow-500/30" title="Vertex AI unavailable or disabled; using local lightweight models">
                      Fallback Active
                    </span>
                  )}
                </div>
