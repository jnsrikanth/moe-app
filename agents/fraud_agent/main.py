...
# Optional Vertex AI import (disabled in secure/offline local runs)
...
if USE_VERTEX:
    try:
        ...
        USE_VERTEX = False
        generative_models = None  # type: ignore
...
# ML helper
try:
    from common.ml import load_model, simple_fraud_scores
except Exception:
    load_model = None  # type: ignore
    simple_fraud_scores = None  # type: ignore
...
# Initialize Vertex AI (when enabled)
if USE_VERTEX and vertexai and GenerativeModel:
    try:
        vertexai.init(project=VERTEX_AI_PROJECT, location=VERTEX_AI_LOCATION)
        model = GenerativeModel(MODEL_NAME)
    except Exception as _e:
        logger = logging.getLogger(__name__)
        logger.warning("Vertex AI init failed, falling back to offline mode: %s", _e)
        USE_VERTEX = False
...
class FraudDetector:
    ...
    def __init__(self):
        ...
        if USE_ML and load_model:
            path = os.getenv("FRAUD_MODEL_PATH", "models/fraud.joblib")
            self.ml_model = load_model(path, kind="fraud")
        self.known_patterns = [
    ...
    async def _analyze_transaction(self, request: FraudRequest) -> Optional[TransactionAnalysis]:
        """Extract and analyze transaction details (offline-friendly)."""
        
        if USE_ML and self.ml_model and simple_fraud_scores:
            # Use metadata to fill the transaction analysis skeleton
            fs = simple_fraud_scores(request.metadata)
            return TransactionAnalysis(
                transaction_id="TXN-ML",
                amount=float(request.metadata.get("amount", 100.0)),
                merchant=str(request.metadata.get("merchant", "Unknown")),
                location=str(request.metadata.get("location", "N/A")),
                timestamp=datetime.now(),
                risk_score=float(fs["risk_score"]),
                anomaly_flags=fs["patterns"],
            )
        
        if not USE_VERTEX or not self.model:
            return TransactionAnalysis(
                transaction_id="TXN-OFFLINE",
                amount=float(request.metadata.get("amount", 100.0)),
                merchant=str(request.metadata.get("merchant", "Unknown")),
                location=str(request.metadata.get("location", "N/A")),
                timestamp=datetime.now(),
                risk_score=0.35,
                anomaly_flags=["offline_default"]
            )
...
If transaction details cannot be extracted, return null."""
    ...
    async def _check_fraud_indicators(
        ...
        """Check for various fraud indicators (offline-friendly)."""
        
        if USE_ML and self.ml_model and simple_fraud_scores:
            fs = simple_fraud_scores(request.metadata)
            return FraudIndicators(
                velocity_check=bool(request.metadata.get("recent_txn_count", 0) > 5),
                location_anomaly=bool(float(request.metadata.get("geo_distance_km", 0.0)) > 1000),
                amount_anomaly=bool(float(request.metadata.get("amount", 0.0)) > 1000),
                merchant_risk=False,
                pattern_match=bool(len(fs["patterns"]) > 0),
...
{{
    ...
    async def _detect_patterns(
        ...
        """Detect known fraud patterns (offline-friendly)."""
