...
# ML helper
try:
    from common.ml import load_model, simple_credit_score
except Exception:
...
class CreditAnalyzer:
    ...
    def __init__(self):
        ...
        if USE_ML and load_model:
            path = os.getenv("CREDIT_MODEL_PATH", "models/credit.joblib")
            self.ml_model = load_model(path, kind="credit")
    ...
    async def _calculate_credit_score(self, request: CreditRequest) -> CreditScore:
        """Calculate credit score using ML when available; otherwise offline-friendly defaults or Vertex."""
        
        # Prefer offline ML
        if USE_ML and self.ml_model and simple_credit_score:
            data = simple_credit_score(request.metadata)
            return CreditScore(
                score=data["score"],
                rating=data["rating"],
        ...
        if not USE_VERTEX or not self.model:
            ...
            return CreditScore(
                ...
                rating=rating,
                confidence=0.6,
                factors=["Offline mode", "Heuristic default"]
            )
...
{{
    ...
    async def _assess_risk(self, request: CreditRequest, credit_score: Optional[CreditScore]) -> RiskAssessment:
        """Assess credit risk (offline-friendly)."""
...
{{
    ...
    async def _check_loan_eligibility(
        ...
        """Check loan eligibility (offline-friendly)."""
        ...
        if not USE_VERTEX or not self.model:
            ...
            return LoanEligibility(
                ...
                conditions=["Offline mode default"]
            )
